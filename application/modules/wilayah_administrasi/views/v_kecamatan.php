...
<div class="header">
    <div class="header-content">
        ...
        <ul class="breadcrumb">
            ...
            <li class="active"><?=ucwords(strtolower($kota['kota']));?></li>
        </ul>
    </div>
</div>
    ...
    <div class="panel panel-default panel-bordered">
        <div class="panel-heading">
            ...
            <?php if ($privileges[0] == 1): ?>
                <div class="elements">
                    <button type="button" id="add-kecamatan" class="btn btn-sm btn-info">Tambah <i class="icon-plus2"></i></button>
                </div>
            <?php endif; ?>
        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        var oTable = $('#tbl-kecamatan').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url('wilayah_administrasi/dt_kecamatan'); ?>',
            "bAutoWidth": false,
            ...
            "fnServerData": function(sSource, aoData, fnCallback)
            {
                ...
                aoData.push({"name": "kota_id", "value": "<?php echo $this->uri->segment(3); ?>"});
        ...
        $('#add-kecamatan').on('click', function(e) {
            window.location.href = "<?=base_url('wilayah_administrasi/add/kecamatan/'.$this->uri->segment(3));?>";
        });

        $('#tbl-kecamatan').delegate('a.btn-delete', 'click', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            swal({
                ...
                function(isConfirm){
                    if (isConfirm){
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?=base_url('wilayah_administrasi/delete/kecamatan');?>",
                            data: {id: id},
                            success: function(r){
