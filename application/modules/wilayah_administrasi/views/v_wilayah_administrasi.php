    ...
    <div class="panel panel-default panel-bordered">
        <div class="panel-heading">
            ...
            <?php if ($privileges[0] == 1): ?>
                <div class="elements">
                    <button type="button" id="add-propinsi" class="btn btn-sm btn-info">Tambah <i class="icon-plus2"></i></button>
                </div>
            <?php endif; ?>
        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        var oTable = $('#tbl-propinsi').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url('wilayah_administrasi/dt_propinsi'); ?>',
            "bAutoWidth": false,
        ...
        $('#add-propinsi').on('click', function(e) {
            window.location.href = "<?=base_url('wilayah_administrasi/add/propinsi');?>";
        });

        $('#tbl-propinsi').delegate('a.btn-delete', 'click', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            swal({
                ...
                function(isConfirm){
                    if (isConfirm){
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?=base_url('wilayah_administrasi/delete/propinsi');?>",
                            data: {id: id},
                            success: function(r){
