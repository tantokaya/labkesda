...
<div class="header">
    <div class="header-content">
        ...
        <ul class="breadcrumb">
            ...
            <li class="active">Propinsi <?=ucwords(strtolower($propinsi['propinsi']));?></li>
        </ul>
    </div>
</div>
    ...
    <div class="panel panel-default panel-bordered">
        <div class="panel-heading">
            ...
            <?php if ($privileges[0] == 1): ?>
                <div class="elements">
                    <button type="button" id="add-kota" class="btn btn-sm btn-info">Tambah <i class="icon-plus2"></i></button>
                </div>
            <?php endif; ?>
        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        var oTable = $('#tbl-kota').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url('wilayah_administrasi/dt_kota'); ?>',
            "bAutoWidth": false,
            ...
            "fnServerData": function(sSource, aoData, fnCallback)
            {
                ...
                aoData.push({"name": "propinsi_id", "value": "<?php echo $this->uri->segment(3); ?>"});
        ...
        $('#add-kota').on('click', function(e) {
            window.location.href = "<?=base_url('wilayah_administrasi/add/kota/'.$this->uri->segment(3));?>";
        });

        $('#tbl-kota').delegate('a.btn-delete', 'click', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            swal({
                ...
                function(isConfirm){
                    if (isConfirm){
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?=base_url('wilayah_administrasi/delete/kota');?>",
                            data: {id: id},
                            success: function(r){
