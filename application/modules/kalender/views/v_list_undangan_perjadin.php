    ...
    $(function(){
        ...
        $('#list_undangan').delegate('a.btn-delete-peserta', 'click', function(e){
            ...
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "<?=base_url('kalender/delete_peserta');?>",
                data: {id: id},
                success: function(r){
                    getListUndangan($('#kegiatan_id').val());
                    new PNotify({
                        title: r.type == 'error'?'Error':'Sukses',
                        text: r.message,
                        icon: r.type == 'error'?'icon-blocked':'icon-checkmark3',
                        type: r.type,
                        addclass: r.type == 'error'?'bg-danger':'bg-success'
                    });
                }
            });
            ...
            $('#modal_title_undangan').html('Ubah Peserta Kegiatan ' + title);

            $.ajax({
                type: "POST",
                url: "<?=base_url('kalender/form_peserta_edit');?>",
            ...
            var title = ($('#jenis_kegiatan_id').val() == '3')?'Perjadin':'FGD / Rapat / Seminar';
            $('#modal_title_undangan').html('Lihat Data Peserta Kegiatan ' + title);

            $.ajax({
                type: "POST",
                url: "<?=base_url('kalender/form_peserta_view');?>",
                data: {id: id, jenis_kegiatan_id: $('#jenis_kegiatan_id').val()},
                success: function (result) {
                    $('#modal_content_undangan').html(result);
                },
