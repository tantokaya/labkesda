...
<script type="text/javascript">
    $(document).ready(function() {
        var oTable = $('#tbl-jenis-kegiatan').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url('jenis_kegiatan/dt_jenis_kegiatan'); ?>',
            "bAutoWidth": false,
            ...
            "columns": [
                {"data":"jenis_kegiatan", "sName":"jenis_kegiatan","sWidth":"65%"},
                {"data":"warna_jenis_kegiatan", "sName":"warna_jenis_kegiatan", "sWidth":"30%"},
        ...
        $('#btn-add').on('click', function(e) {
            window.location.href = "<?=base_url('jenis_kegiatan/add');?>";
        });

        $('#tbl-jenis-kegiatan').delegate('a.btn-delete', 'click', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            swal({
                ...
                function(isConfirm){
                    if (isConfirm){
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?=base_url('jenis_kegiatan/delete');?>",
                            data: {id: id},
                            success: function(r){
