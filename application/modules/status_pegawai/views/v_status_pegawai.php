...
<script type="text/javascript">
    $(document).ready(function() {
        var oTable = $('#tbl-status-pegawai').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url('status_pegawai/dt_status_pegawai'); ?>',
            "bAutoWidth": false,
            ...
            "columns": [
                {"data":"status_pegawai", "sName":"status_pegawai"},
        ...
        $('#btn-add').on('click', function(e) {
            window.location.href = "<?=base_url('status_pegawai/add');?>";
        });

        $('#tbl-status-pegawai').delegate('a.btn-delete', 'click', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            swal({
                ...
                function(isConfirm){
                    if (isConfirm){
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?=base_url('status_pegawai/delete');?>",
                            data: {id: id},
                            success: function(r){
