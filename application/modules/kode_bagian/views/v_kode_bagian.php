...
<script type="text/javascript">
    $(document).ready(function() {
        var oTable = $('#tbl-kode-bagian').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url('kode_bagian/dt_kode_bagian'); ?>',
            "bAutoWidth": false,
            ...
            "columns": [
                {"data":"kode_bagian", "sName":"kode_bagian", "sWidth":"25%"},
                {"data":"deskripsi", "sName":"deskripsi", "sWidth":"70%"},
        ...
        $('#btn-add').on('click', function(e) {
            window.location.href = "<?=base_url('kode_bagian/add');?>";
        });

        $('#tbl-kode-bagian').delegate('a.btn-delete', 'click', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            swal({
                ...
                function(isConfirm){
                    if (isConfirm){
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?=base_url('kode_bagian/delete');?>",
                            data: {id: id},
                            success: function(r){
