...
<script type="text/javascript">
    $(document).ready(function() {
        var oTable = $('#tbl-pengaturan-laporan').dataTable( {
            "bProcessing": true,
            "bServerSide": true,
            "sAjaxSource": '<?php echo base_url('pengaturan_laporan/dt_pengaturan_laporan'); ?>',
            "bAutoWidth": false,
            ...
            "columns": [
                {"data":"jenis_kegiatan", "sName":"jenis_kegiatan", "sWidth":"25%"},
                {"data":"jenis_laporan", "sName":"jenis_laporan", "sWidth":"70%"},
        ...
        $('#btn-add').on('click', function(e) {
            window.location.href = "<?=base_url('pengaturan_laporan/add');?>";
        });

        $('#tbl-pengaturan-laporan').delegate('a.btn-delete', 'click', function(e){
            e.preventDefault();
            var id = $(this).data('id');
            swal({
                ...
                function(isConfirm){
                    if (isConfirm){
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: "<?=base_url('pengaturan_laporan/delete');?>",
                            data: {id: id},
                            success: function(r){
